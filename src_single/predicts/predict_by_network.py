...
def evaluate(X_predict):
    with tf.Graph().as_default() as g:
        # 网络输入、输出tensor
        x = tf.placeholder(tf.float32,shape=(None,network.INPUT_NODE),name='x_input')
        y = network.inference(x,avg_class=None,reuse=False,lamada=None) #预测结果
...
def main(argv=None):
    ...
    for filename in os.listdir(predict_path):
        ...
        X_predict = img.reshape(-1, IMG_HEIGHT * IMG_WIDTH) # 转化成模型输入形状
